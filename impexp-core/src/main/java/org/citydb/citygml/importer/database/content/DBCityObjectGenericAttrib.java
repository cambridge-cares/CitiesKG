		...
		if (genericAttribute.getCityGMLClass() == CityGMLClass.GENERIC_ATTRIBUTE_SET) {
			...
			case STRING_ATTRIBUTE:
				...
				if (stringAttribute.isSetValue()) {
					// Note: for postGIS or blazegraph, this step is same.
				...
				if (isBlazegraph){
					...
					ps.setNull(++index, Types.NULL);  //4
					ps.setNull(++index, Types.NULL);  //5
					ps.setNull(++index, Types.VARCHAR);  //6
					ps.setNull(++index, Types.DATE);  //7
					ps.setNull(++index, Types.VARCHAR);  //8
					}
				break;
			case INT_ATTRIBUTE:
				ps.setInt(++index, 2);
