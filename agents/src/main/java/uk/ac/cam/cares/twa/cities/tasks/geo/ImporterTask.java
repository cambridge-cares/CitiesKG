...
/**
 * Runnable task importing a CityGML model into an instance of a SPARQL sever picked up from the
 * {@link BlockingQueue}. It uses an instance of {@link ImpExp} configured before starting the
 * import of a specific model CityGML file. The server instance and the task is stopped after the
 * import process is finished and a temporary .nq file is created to indicate that the .jnl file is
 * ready for processing by other tasks.
 *
...
public class ImporterTask implements Runnable {
      ...
      while (!serverInstances.isEmpty()) {
        try {
          ...
          File nqFile = new File(
              importFile.getAbsolutePath().replace(EXT_FILE_GML, EXT_FILE_NQUADS));
  ...
  /**
   * Replaces placeholder values in Importer/Exporter project config file with the ones of the
   * running local instance of the Blazegraph server.
   *
  ...
  private File setupFiles(URI endpointUri) throws URISyntaxException, IOException {
    ...
    Files.copy(Paths.get(
            Objects.requireNonNull(getClass().getClassLoader().getResource(PROJECT_CONFIG)).toURI()),
    ...
    cfgData = cfgData.replace(PLACEHOLDER_NS,
        URL_BLZG_NS + BlazegraphServerTask.NAMESPACE + URL_SPARQL);
