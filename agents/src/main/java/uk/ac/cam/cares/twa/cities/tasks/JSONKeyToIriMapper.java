...
/**
 * Class to retrieve and store a mapping from JSON key to knowledge graph IRI which extends a HashMap with fixed types.
...
public class JSONKeyToIriMapper {
    ...
    private boolean checkIRI(String iri) {
        ...
        return Pattern.compile("\\w+\\S+:\\S+\\w+").matcher(iri).find();
    }
}
